...
public class EntityRenderer implements IResourceManagerReloadListener
{
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    f4 = f4 * 0.1F;
                    f5 = f5 * 0.1F;
                    MovingObjectPosition movingobjectposition = this.mc.theWorld.rayTraceBlocks(new Vec3(d0 + (double)f3, d1 + (double)f4, d2 + (double)f5), new Vec3(d0 - d4 + (double)f3 + (double)f5, d1 - d6 + (double)f4, d2 - d5 + (double)f5), false, true, false);
    ...
    {
        if (!Config.isCloudsOff() && Shaders.shouldRenderClouds(this.mc.gameSettings))
        {
            GlStateManager.matrixMode(5889);
            GlStateManager.loadIdentity();
